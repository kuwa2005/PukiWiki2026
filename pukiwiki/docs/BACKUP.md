...
|------|------|
| `index.php` | エントリポイント |
| `pukiwiki/` | Wiki 本体（lib, plugin, skin, image, ini, wiki, cache, attach, docs, tools 等）＋**公式同梱**（`README.txt`, `INSTALL.txt`, `COPYING.txt`, `UPDATING.txt`, `*.en.txt.zip`, `wiki.en.zip`） |

**含めないもの（git リポジトリ root のみ）:**

- `README.md`, `CHANGELOG.md` — プロジェクト文書（git 管理、バックアップ任意）
- `.github/` — CI 設定
...
4. `pukiwiki/wiki/`・`pukiwiki/cache/` 等の書き込み権限を確認（Unix/Linux 本番では起動時パーミッションチェックが自動実行 — [DEPLOY.md §3.2](DEPLOY.md#32-パーミッション)）

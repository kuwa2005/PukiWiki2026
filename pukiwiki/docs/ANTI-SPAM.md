...
1. **`$auth_users` に編集者を1件以上登録**
   ...
   雛形にはデモ用の初期ユーザーが含まれます。**必ず変更してから使うこと。** 初回ログイン時にパスワード変更 UI が表示されます。
...
3. 初回ログイン（`editor` / `editor`）でパスワード変更画面へ誘導されること
4. ログイン後に編集・保存できること
5. 匿名のままページ閲覧ができること
6. 未認証で `curl -X POST` による編集 payload を送っても保存されないこと（ログイン誘導または拒否）
